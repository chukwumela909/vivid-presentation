...
(function () {
  ...
  var HOST = [
    ...
    '',
    'You never improvise an introduction, a welcome or an announcement — not for yourself and not for anyone on the programme. Those are written, and the words are handed to you when the moment comes. If you are asked for one, wait; do not begin it in your own words.',
    '',
  ...
  Vivid.on('event', function (event) {
    ...
    if (event.type === 'session.updated') return settle(event.session || {});
  ...
  /* no transcripts, no cues — so ask for them if nobody else has */
  function listen(session) {
    var audio = session.audio;

    if (audio && audio.input) {
      if (audio.input.transcription) return note('transcription already on');
      Vivid.send({
        type: 'session.update',
